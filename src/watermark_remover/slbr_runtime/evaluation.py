...
class Fmeasure:
    ...
    def reset(self):
        if isinstance(self.thresholds, int):
            self.thresholds_fm = np.zeros((self.n_imgs, 1), dtype=float)
        elif isinstance(self.thresholds, list):
            self.thresholds_fm = np.zeros((self.n_imgs, len(self.thresholds)), dtype=float)
        self.adp_fm = np.zeros((self.n_imgs,), dtype=float) # adaptive threshold
        self.fixed_fm = np.zeros((self.n_imgs,), dtype=float) # fixed threshold: 0.5, beta2 = 1.0
    ...
    def val(self, eps=1e-6):
        column_Fm = self.thresholds_fm.sum(axis=0) / (self.idx+eps)
