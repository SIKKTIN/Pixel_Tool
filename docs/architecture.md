...
棋盘格背景去除位于 `src/perfect_pixel/fake_checkerboard.py`，通过周期和边缘连通性识别写入 RGB 的假透明背景，输出 RGBA；桌面端和 MCP 共用同一算法。

桌面窗口的工作区由主面板、当前工具操作区、缓存区和底部项目资产栏组成。顶部 Tab 继续负责模块切换；空格键切换资产栏显隐，资产栏浏览项目目录并可双击把图片载入当前模块。
